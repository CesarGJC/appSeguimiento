...
public class OperacionesController {
    ...
    public String index(Model modelo) {
        int idPlanPei = Integer.parseInt(actividadesFacade.getValorConfiguracionPorClave("id_plan_pei"));
        List<FormularioResponse> formularioResponsesList = actividadesFacade.getFormulariosPorPlan(idPlanPei);
    ...
    public String redirigir(@ModelAttribute("id") Integer id, RedirectAttributes redirectAttributes) {
        redirectAttributes.addAttribute("id", id);
        if (getUsuario().getId_programa() != 0 & getUsuario().getId_departamento() != 0) {
            return "redirect:/operaciones/seleccionar-tipo";
        }
        if (getUsuario().getId_facultad() != 0 & getUsuario().getId_departamento() != 0) {
            return "redirect:/operaciones/seleccionar-tipo";
        }
        if (getUsuario().getId_programa() != 0 && getUsuario().getId_departamento() == 0) {
    ...
    public String nuevo(@ModelAttribute("model") OperacionesRequest model, Model modelo) {
        model.setId_detalle_periodos_programacion(Integer.parseInt(actividadesFacade.getValorConfiguracionPorClave("id_periodo_gestion")));
