...
function registrarDesignacion(input) {
    ...
    let data = {
        ...
        id_departamento: parseInt(document.getElementById('id_departamento').value),
        id_tipo_asignacion_ant: 0,
        id_asignacion: 0,
...
let seleccionarElemento = (input) => {
    ...
    let data = {
        ...
        id_desconcentrado: isNaN(parseInt(input.dataset.id_desconcentrado))?-1:parseInt(input.dataset.id_desconcentrado)
    };
    Get(url, data)
        .then(function (result) {
            document.getElementById('formulario').innerHTML = result;
